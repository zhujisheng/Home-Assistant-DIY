...
- 安装好的HomeAssistant（我使用树莓派上HAOS）
- 一台性能较好的计算机（我使用Ubuntu系统）
- 两者都能访问到的一个文件系统(我使用ssh文件访问，*请避免使用samba*)

ssh共享文件的操作实践：

1. 安装启动add-on：Terminal&SSH
...
2. 创建文件`$~/.ssh/id_rsa`
    ...
    `touch $~/.ssh/id_rsa && chmod 500 $~/.ssh/id_rsa`
